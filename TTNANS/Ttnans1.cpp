...
if(TGBL no_error)                      /* It does not change NODE.next . */
...
if(TGBL no_error)                      /* It does not change LINK.next . */
 {
 cp1=cp2=lp->links;
...
if(TGBL no_error)                           /* It does not change LAYER.next. */
 {
 lp1=lp2=ap->layers;
...
if(TGBL no_error)                           /* It does not change ANS.next.   */
 {
 ap1=ap2=*al;
...
if(TGBL no_error)
 ...
 if(TGBL no_error)
  ...
  ap->inputlayer=NULL;
  ap->outputlayer=NULL;
  ap->layers=NULL;                     /* does not contain any layers yet.*/
...
if(np==NULL)
 ...
 mprintf("\t.With Processing Elements:\n");
...
if((TGBL no_error)&&(ap!=NULL)&&(showans))
 {
 if(TGBL show_ans_option)
  {
  mhintclear();
  mhintf(false,"Note: You are asked whether or not to show the ANS structure (Static)\n");
  mhintf(false,"If you select not to show the ANS structure now you will never be\n");
  mhintf(false,"asked again for the remainder of this run.\n");
  mhintf(false," \n");
...
extern void ReportToSessionFile(const char * x);
