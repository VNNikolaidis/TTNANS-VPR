...
if((i<=0)||
   ((fname[i+1]!='s')&&
    ...
    (fname[i+1]!='X')))
 {
 mprintf("Warning: invalid filename (%s) for class (cluster) file.\n",fname);
 mprintf("(Valid filenames are : *.S*, *.F*, *.X*)\n");
 return false;
 }
...
RemoveFilenameExtension(target);
strcat(target,".cla");
}
...
{
if(TGBL no_error&&is_class_file(original))
...
if(is_class_file(fname))
 ...
 mprintf("Using preselected scaling method : <");
...
mhintclear();
mhintf(showinfo,"This option allows you to make class files (*.f,*.x) using   \n");
mhintf(showinfo,"a descriptor set (defined in a .PRB file) other than the one \n");
mhintf(showinfo,"with which the originals were created. For example, if some  \n");
mhintf(showinfo,"clustering method A used to produce class files (A.x*,A.f*)  \n");
mhintf(showinfo,"with the descriptor set Aset, needs to be compared with      \n");
mhintf(showinfo,"clustering B (applied to the same data sets and using Bset)  \n");
mhintf(showinfo,"then this method can convert A.f* full descriptor class files\n");
mhintf(showinfo,"to ones where only Bset descriptors are used.                \n");
mhintf(showinfo," \n");
mhintf(showinfo,"Note :- Class numbering may change.                          \n");
mhintf(showinfo,"      - Multiple entries of the same descriptor in Bset are  \n");
mhintf(showinfo,"        ignored and skipped.                                 \n");
mhintf(showinfo,"      - Relative order between decriptors (cols) in original \n");
mhintf(showinfo,"        files is preserved (no re-ordering).                 \n");
mhintf(showinfo," \n");
showinfo=false;
...
mhintclear();
mhintf(showinfo,"This routine restores class files (*.f,*.x) using the\n");
mhintf(showinfo,"information stored in the classifation (*.CLA) file.\n");
mhintf(showinfo,"Note: Sets currently in memory will be erased.\n");
mhintf(showinfo," \n");
showinfo=false;
...
RemoveFilenameExtension(clafbase);
...
if((TGBL no_error))
 ...
 if(prbinfo->problem_type==PT_CLUSTERING)       /* Get pseudo-testset size.*/
  GetDataFileDims(prbinfo->trn_file,            /* if PT_CLUSTERING, check */
		  &(TGBL testsetsize),&(TGBL inputdim));
 else                                           /* original trainset sizes */
  GetDataFileDims(prbinfo->tst_file,
		  &(TGBL testsetsize),&(TGBL inputdim));  /* else testset sizes.   */
 if(((TGBL no_error))&&
    (cols==1)&&
    (rows==(TGBL testsetsize)))
  {
  (TGBL inputdim)=prbinfo->descriptor_dim;
  CreateClassif();
  ...
  if((TGBL no_error))
   ...
   if(FileOpenOK(f))
    ...
    for(i=0;(i<rows)&&(TGBL no_error);i++)
     {
     (TGBL classif)[i]=ReadDATA(f);									/* this is not elegant */
     if((TGBL classif)[i]>maxclass) maxclass=(TGBL classif)[i];		/* but we need it to   */
     }																/* make (TGBL outputdim) for  */
    CloseFile(f);													/* SaveClasses() call. */
    (TGBL outputdim)=(int)maxclass+1;
  ...
  {
  mprintf("Can't continue.(Possible cause: incompatible *.cla size: %s (%d x %d)).\n",clafname,rows,cols);
  }
 }
FreePrbInfo(prbinfo);
FreeAllData();
