 ...
 *max_t=0;
 while(*max_t<=0)
  {
  mprintf("Select iteration limit,(originaly 10000).\n");
  *max_t=mrequest_int("Select max iteration limit.");
  }

 *z=0;
 while((*z<=0)||(*z>*max_t))
  {
  mprintf("Select z ((max iteration to run, originaly 500),\n");
  mprintf("          (0 < z =< iteration limit)).\n");
 ...
 *e=-1.0;
 while(*e<0.0)
  {
  mprintf("Error level Et is defined as the difference in weight values\n");
  mprintf("of connection weights (i.e. movement of cluster centers) after\n");
  mprintf("a training cycle has been completed; it is used as a convergence\n");
  mprintf("measure (i.e. to determine if training should continue).\n");
 ...
 *a=0.0;
 while((*a<=0.0)||(*a>=1))
  {
  mprintf("Select a ((learning rate factor),(0<a<1),(originaly 0.2).\n");
 ...
 *n=0;
 while((*n<1)||(*n>outs))
  {
  mprintf("Select n (neighbourhood size) (1=<n=<%d).\n",outs);
 ...
 *nstep=-1.0;
 while((*nstep<0)||(*nstep>(DATA)*n))
  {
  mprintf("Select nstep (neighbourhood decreament step size).\n");
...
if((Trainset!=NULL)&&(Testset!=NULL)&&(no_error))
 {
